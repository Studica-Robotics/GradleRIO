...
class FRCJavaArtifact extends JavaArtifact {
    ...
    def robotCommand = {
        "/usr/local/frc/JRE/bin/java -XX:+UseG1GC -XX:MaxGCPauseMillis=25 -Djava.library.path=${FRCPlugin.LIB_DEPLOY_DIR} -Djava.lang.invoke.stringConcat=BC_SB ${jvmArgs.join(" ")} ${debug ? debugFlags : ""} -jar \"<<BINARY>>\" ${arguments.join(" ")}"
    }
